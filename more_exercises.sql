...
from category
where category_id > 4
and name like 'c%' or 'S%' or 'T%';

# Select all columns minus the password column from the staff table for rows that contain a password.
select * #write down all columns except for password
from staff 
...
from address
where address in ('california','england','taipei','west java');
...
select payment_id, amount, payment_date
from payment
where payment_date in ('05/25/2005', '05/27/2005','05/29/2005');
...
# World Database (Use the world database for the questions below) #
...
# Advanced: Pizza Database (Use the pizza database to answer the following questions)
